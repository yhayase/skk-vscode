...
export abstract class AbstractKanaMode implements InputMode {
    ...
    protected abstract generateAllOkurigana(): string[];
    protected abstract generateAllGokan(): string;
    protected abstract generateWholeYomigana(): string;
    ...
    private doHenkan(okuri: string | undefined = undefined) {
        ...
        if (okuri) {
            const sagyo = this.generateAllOkurigana();
            if (midashigo === this.generateAllGokan() && sagyo.includes(okuri[0])) {
            ...
            if (midashigo === this.generateWholeYomigana()) {
