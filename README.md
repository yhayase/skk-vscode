...
- **Web Extensions 対応**: VS Code for the Web（vscode.dev / github.dev）を含むブラウザ環境でも利用可能です。
...
- **Web Extensions 対応**: ブラウザ版 VS Code（vscode.dev や github.dev）上でも拡張機能を利用できるようになりました（Webpack によるバンドル構成と Web 標準 API への移行）。
- **リリース自動化**: VS Code Marketplace への公開および Git タグ作成に連動した GitHub Releases 自動発行パイプラインを整備しました。
...
- **テスト基盤の強化**: ヘッドレス環境における統合テストの安定化と、ブラウザ環境向け自動テスト（`@vscode/test-web`）を追加しました。

#### バグ修正
- **コマンド排他制御と非同期処理の適正化**: 入力ハンドラの非同期処理が未待機のまま完了していた不具合を解消し、高速打鍵時やモード遷移時にキー入力順序や状態が乱れるレースコンディションを修正しました。
- **Abbrevモードの辞書登録不具合の修正**: Abbrevモード（スラッシュ入力による検索）から辞書登録画面を開いた際の表示や動作の不具合を修正しました。
